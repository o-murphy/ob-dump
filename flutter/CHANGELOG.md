...
- **Now a real Flutter plugin (`ffiPlugin: true`) for all five platforms
  ...
  `flutter_lmdb2` dependency anymore — it has no Linux/Windows support
  at all as published, and provided no benefit here now that this
  package compiles and bundles the library itself directly. Uses
  `ffiPlugin: true` rather than `pluginClass:` since there's no method-
  channel surface at all — confirmed against the official
